-- ============================================================
-- FIALN / STUDENT STATUS — Single Source of Truth via VIEW
-- Migration: 20260915183000_fialn_student_status_view.sql
-- ============================================================

-- 1. Create VIEW public.fialn_students_view
-- Rules:
--   - pendente: sem email confirmado OU termos não aceitos (com ou sem matrícula)
--   - ativo: email confirmado + termos aceitos + matrícula ativa vigente (status = 'active' and (end_date IS NULL or end_date >= CURRENT_DATE))
--   - inativo: email confirmado + termos aceitos + sem matrícula ativa vigente
CREATE OR REPLACE VIEW public.fialn_students_view
...
SELECT
  ...
  CASE
    WHEN sp.email_verified_at IS NULL OR sp.terms_accepted_at IS NULL THEN 'pendente'
    WHEN EXISTS (
      SELECT 1
...
GRANT SELECT ON public.fialn_students_view TO authenticated, service_role;

-- 2. Drop physical column status and constraint from fialn_student_profiles
ALTER TABLE public.fialn_student_profiles
  DROP CONSTRAINT IF EXISTS fialn_student_profiles_status_check;

ALTER TABLE public.fialn_student_profiles
  DROP COLUMN IF EXISTS status CASCADE;

-- 3. Update register_student_public to remove column status insert
...
-- 4. Update confirm_student_registration to not touch status
...
-- 5. Update get_student_registration_by_token to select from fialn_students_view
