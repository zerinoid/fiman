-- ============================================================
-- FIALN / STUDENT CONFIRMATION — Email Verification by Token
-- Migration: 20260910020000_fialn_email_confirmation.sql
-- ============================================================

-- 1. Add email confirmation columns to fialn_student_profiles
ALTER TABLE public.fialn_student_profiles
  ADD COLUMN IF NOT EXISTS confirmation_token UUID DEFAULT gen_random_uuid(),
  ADD COLUMN IF NOT EXISTS confirmation_token_expires_at TIMESTAMPTZ DEFAULT (NOW() + INTERVAL '72 hours'),
  ADD COLUMN IF NOT EXISTS email_verified_at TIMESTAMPTZ;

-- Index for fast token lookups
CREATE INDEX IF NOT EXISTS idx_fialn_student_profiles_confirmation_token
  ON public.fialn_student_profiles (confirmation_token)
  WHERE confirmation_token IS NOT NULL;

-- 2. Update register_student_public to generate and return confirmation_token
...
BEGIN
  ...
  -- 6. Insert into fialn_student_profiles with audit trail and token
...
-- 3. RPC to read registration by token for student review
...
-- 4. RPC to confirm student email registration
...
-- Permissions
REVOKE EXECUTE ON FUNCTION public.get_student_registration_by_token(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_student_registration_by_token(UUID) TO anon, authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.confirm_student_registration(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.confirm_student_registration(UUID) TO anon, authenticated, service_role;
